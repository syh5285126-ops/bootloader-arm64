...
> 本轮（第五轮）有两个并列候选假设：H1（`unsigned long` 截断 64 位 DMA
> 地址）和 H2（V4/64 位寻址模式实际未在硬件生效，DMA 写错了寄存器/地址）。
> 已经在 `bm1684xSdhciSendCmd()` 里加了一段合并诊断 `printk`，跑一次硬件
> 就能同时区分 H1/H2，等用户反馈 CMD8 那行日志。另有一个低置信度怀疑点
> （PLL_EN 缺失）已经改了代码但逻辑上解释不通毒药现象，不是当前主攻方向。
...
**两个并列的当前候选假设（H1、H2），尚未经硬件证据区分**：

- **H1：`unsigned long` 截断 64 位 DMA 地址**（原本认为优先级最高，现降级
  为与 H2 并列）。`bm1684xSdhci.c:535` 一带：
  ```c
  ...
  ```
  如果这套 VxWorks/ACoreOS 工具链里 `unsigned long` 是 **32 位**，第一步
  `(unsigned long)pData->buf` 就已经把 64 位指针截断成 32 位，导致
  `ADMA_SA_HIGH` 永远写 0、真实地址高位被静默丢弃。**已经问用户、还没
  拿到答案**：`unsigned long` 在这套工具链里的实际宽度
  （`sizeof(unsigned long)`）——纯编译环境事实，不需要上板。
- **H2：V4/64 位寻址模式实际并未在硬件上生效**（第五轮新发现，目前更受
  关注）。对照用户上传的 ref1 `bm_sd.c`（约第 553 行）`bm_sd_hw_init()`，
  发现它在置位 `HOST_CONTROL2` 的 64 位寻址位之前，会先检查
  `SDHCI_CAPABILITIES1` 的 bit27（控制器是否真的支持 64 位 V4 模式），
  而本驱动 `hwInit()`（`bm1684xSdhci.c` 约第 257-309 行）**无条件**根据
  `pDev->is64Bit` 置位 HC2 寄存器，完全没有读 CAPABILITIES1 做校验。如果
  这块板子的控制器硬件实际不支持/未使能 64 位模式，HC2 的 64BIT 位写入
  会被硬件忽略，控制器仍按**legacy 32 位 SDMA 模式**解释寄存器布局——
  这时 `SDHCI_DMA_ADDRESS`（偏移 0x00）就不是"复用为 32 位块计数寄存
  器"，而是**仍然是真正的 SDMA 地址寄存器**。但驱动代码此时已经按 64 位
  模式的假设把 `pData->blkCount`（值为 1）写进了 0x00 这个寄存器——如果
  硬件实际跑在 legacy 模式，DMA 引擎就会把数据写到物理地址 `0x1`，而不是
  `bounceBuf`，控制器仍然正常完成传输并报告成功。这能完整解释"控制器报
  成功、但 CPU 通过完整 64 位指针读到的 `bounceBuf` 完全没被触碰"的现象，
  且不需要任何"地址截断"假设。
- 两者都能解释毒药现象，区别在于：H1 是地址高位丢失（只有物理地址超
  4GB 才触发），H2 是寄存器布局/模式判断本身就错了（不管地址是否超
  4GB 都会触发）。**新增的合并诊断（见下）能在一次硬件运行里同时证伪/
  证实两者，不需要分两轮测**。

### 新增：合并诊断（已写入代码，尚未上板验证）

为避免"一个假设烧一次板"的低效排查方式（用户已就此明确表达不满），在
`bm1684xSdhciSendCmd()` 的数据命令分支里（`buildXferMode()` 调用之后，
写 `SDHCI_ARGUMENT`/发出命令之前）新增了一段一次性读回多个寄存器的诊断
`printk`，单条日志同时回答：
- `HOST_CONTROL2` 的 `VER4_ENABLE`(bit12)/`64BIT_ADDR`(bit13) 是否真的被
  置上（直接验证 H2：模式有没有生效，不是只看驱动自己 `is64Bit` 标志）。
- `SDHCI_DMA_ADDRESS`(0x00)/`ADMA_SA_LOW`/`ADMA_SA_HIGH` 三个寄存器的
  实际内容（直接验证地址有没有写对寄存器、高位有没有被截断丢失）。
- `pData->buf` 完整 64 位指针值 + `sizeof(unsigned long)`/`sizeof(void*)`
  （直接回答 H1 需要的工具链事实，不用再单独问用户）。

下一步硬件验证只需要跑一次 `emmcCardIdentify()`，把 CMD8 对应那一行
`eMMC: dma diag(cmd8): ...` 完整日志发回来即可同时排除/确认 H1 和 H2。

### 当前未提交改动（工作区里，未 commit/push，等用户确认）

1. `bm1684xSdhci.c::bm1684xSdhciSetClk()` —— 补 `SDHCI_CLK_PLL_EN` 位 +
   延时从 150us 改 400us（上面第五轮弯路 3）。**无副作用，建议保留，但
   逻辑上解释不通毒药现象，不是当前主攻方向**。
2. `bm1684xSdhci.c::bm1684xSdhciSendCmd()` —— 新增的合并诊断 `printk`
   （见上一节）。**这是当前排查的关键，下一轮硬件反馈就靠它**。

（`bm1684xEmmcBlkDrv.c` 里的 delta 诊断已作为 commit `61832d22` 提交，
结论已知不可信但代码无害，留着不影响后续排查。）

### 下一步（按优先级）

1. **等用户跑一次硬件，反馈 CMD8 那行 `eMMC: dma diag(cmd8): ...` 完整
   日志**。根据 HC2 的 VER4/64BIT 位是否置上即可立刻判定 H2 是否成立；
   同时 `sizeof(unsigned long)` 立刻回答 H1。两者一次性验证，不需要分轮。
2. 根据诊断结果实施针对性修复：
   - 若 H2 成立（64 位模式未生效）：需要在 `hwInit()` 里仿照 ref1 读
     `CAPABILITIES1` bit27 判断硬件是否真正支持 64 位模式，不支持则
     `pDev->is64Bit` 强制为 0 并切换整条数据路径到 legacy 32 位 SDMA
     寄存器布局（不能只是不写 HC2 位，因为后续 `bm1684xSdhciSendCmd()`
     里所有"是否写 ADMA_SA_LOW/HIGH 还是写 0x00"的分支都依赖这个标志）。
   - 若 H1 成立（`unsigned long` 是 32 位）：把 `bm1684xSdhci.c` 里所有
     "指针→DMA 地址"的转换路径改成显式 64 位类型（参照
     `vxbBm1684xSdhci.c` 的 `PHYS_ADDR` 用法），不依赖 `unsigned long`
     宽度假设。
   - 若两者都不成立（HC2 位确实置上、地址也没截断）：需要重新设计"直接
     证明/排除 DMA 没对目的地址发起写事务"的诊断（比如在另一块已知地址
     埋哨兵值，传输后检查该区域有没有被意外覆盖，定位 DMA 实际落地的
     物理位置），不要回头重复验证已排除的 PHY/cache/虚实地址转换方向。
