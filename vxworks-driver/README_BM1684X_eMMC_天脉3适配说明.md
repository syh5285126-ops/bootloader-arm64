...
### 15.4 新日志拿到：第一条失败 `blkCnt=1024`（正好 512KB）—— 怀疑撞上 SDMA 边界寄存器的硬件
corner case（未证实，已加一项诊断，待下一份日志确认）
最新上板日志（用户截图）：
```
[bm_sd] FAIL WRITE(CMD25) ret=-1 state=0x03f70000 int=0x0000 err=0x0000 clk=0x0207
       resp0=0x00000900 arg=0x00000020 blkcnt=1024 data=WRITE ...
       latched_int=0x00000000 latched_err=0x0(none-latched)
（后续失败全变 ret=-3，arg 每次按 1024 递增，latched_err 解码为 CMD_TIMEOUT_ERROR，
 blkCnt 仍是 1024 —— 跟 15.3 说的"复位后变更差的失败模式"是同一套现象，先看第一条）
```
对照 `pollWaitStatus()` 源码确认：`ret=-1`（`BM_ERR_TIMEOUT`）这条路径**完全不会**碰
`isrStatus`/`isrErrSts`，所以 `latched_int=0`/`latched_err=0` 不是诊断盲区，是这 5 秒等待
窗口内**真的一次中断都没收到**（连 `DMA_END` 边界中断都没有）——`resp0=0x900` 又说明命令
本身已经被卡接收并应答（READY_FOR_DATA=1），问题准确卡在"数据传完之后该来的完成
信号"这一步。

注意到一个高度可疑的规律：**这条、以及目前为止看到的每一条失败都是 `blkCnt=1024`**，正好是
1024×512B = 512KB——而引擎层 `bm1684xSdhci.c` 给 SDMA 配置的"边界寄存器"（`SDHCI_MAKE_BLKSZ`
里的 `7`）经查 TF-A 参考代码 `bm_sd.c` 第 517 行注释明确写着"512K bytes SDMA buffer
boundary"，是同一个值——**怀疑是某种 SDMA 边界硬件 corner case**：当一次传输的长度正好等于
一个边界单位、缓冲区起始地址又正好落在 512KB 整数倍上时，"地址到达边界"和"块数同时归零"两个
条件同时触发，部分 SDHCI 类控制器在这种极端情况下会两个中断都不发（既不是正常完成，也不是
"还有数据、继续"的边界中断），导致主机干等。

这个猜测**目前还停留在数学推导，没有实测证据**——核心缺的一块拼图是：那块失败时用的 DMA
缓冲区，它的地址到底是不是真的卡在 512KB 整数倍上。已在 `bm_sd_core.c` 的失败诊断打印里加上
`buf=`（缓冲区地址）和 `align512k=`（地址对 512KB 取模的余数）两个字段——**如果下一份日志里
这个余数正好是 `0x00000`，基本能确认猜测**；如果不是 0，说明猜错了方向，需要继续查别的可能。
这一步只加了打印，没有改任何读写逻辑，风险为零。

如果余数确认是 0，下一步要做的修复也提前想清楚了：简单把传输拆成两次小块发送**不一定管用**
（拆分后最后一小块的结束地址仍然落在同一个 512KB 整数点上，问题会原样转移过去，不会消失）；
真正可行的办法大概率是加一层"中转缓冲区"——驱动自己另开一块**起始地址故意不对齐 512KB**
的内存，写卡前先把数据 `memcpy` 进去再交给 SDMA，读卡则反过来——这样无论原缓冲区地址在哪，
实际参与 DMA 的地址都不会踩中边界整数点。代价是每次读写多一次整块 `memcpy`（512KB 量级，
按现在 25MHz"求稳"档的传输耗时比例不算大头），但能从根本上避开这个 corner case，不用动
PHY/共用引擎层的寄存器细节。**这只是预案，等下一份日志拿到 `align512k=` 的数值再决定要不要
真的做这个改动**，避免重复前几轮"先认定根因再发现没对上"的弯路。

### 15.5 日志回来：`align512k=0x1d000`（不为 0）——15.4 的猜测被否，但反而锁定了真根因
用户上板日志给出 `align512k=0x1d000`（=118784），**对 512KB 取模不为 0**，所以 15.4"缓冲区
正好压在 512KB 整数点、传输正好落在边界末尾"的猜测**被直接否掉，收回**。

但这个数字把方向指得更准了。重新算一遍这次传输的几何关系：
- 缓冲区起点相对 512KB 边界的偏移 = `0x1d000`，本次传输长度 = `0x80000`（1024×512B = 512KB）。
- 起点偏移 + 长度 = `0x1d000 + 0x80000`，**必然跨过正好一个 512KB 物理边界**（在传输进行到
  `0x80000 - 0x1d000 = 0x63000` 处跨界）。
- 反观 `selftest` 的单块写只有 512 字节，永远跨不过任何 512KB 边界。

**这把一直以来"selftest 单块写能过、格式化大块写卡死"的怪现象解释死了**：差别就在"是否跨越
512KB SDMA 边界"。凡是 ≥512KB 的整块传输都必然要走一次"边界中断(DMA_END)后重载 SDMA 地址
寄存器再续传"的逻辑；单块传输从不走这条路。问题就在这条重载逻辑里。

**真根因（已改，未上板验证）**：逐行比对引擎层 `bm1684xSdhci.c` 的边界重载代码与**两份**独立
参考实现——TF-A `bm_sd.c`（`bm_sd_send_cmd_with_data()` 的 `DMA_END` 分支）和本仓库 VxWorks
版完整驱动 `vxbBm1684xSdhci.c`（中断处理里的 `DMA_END` 分支）——发现：
- 两份参考在 DMA_END 后都是【读回 `ADMA_SA_LOW` 写回 + **再写 `ADMA_SA_HIGH`**】两步；
- 我们的引擎层**只做了第一步**（读写低 32 位），**从没碰高 32 位寄存器 0x5C**。

为什么这一下是致命的：SDMA 到达边界后控制器会暂停并发 DMA_END，必须由软件"重写一次 SDMA
系统地址寄存器"才会越过边界继续——这个【写动作本身】就是续传触发信号。在 V4 **64 位寻址**
模式下，SDMA 地址是 64 位（低位 0x58 + 高位 0x5C），地址的提交/续传触发落在**写高位那一下**。
我们只写了低位、没写高位，于是 64 位模式下"越过边界继续传"始终没被触发，DMA 停在边界处不动，
数据阶段干等到 5 秒超时。这跟日志现象完全对得上（命令已成功 `resp0=0x900`，卡在数据完成信号）。

**一个容易踩反的关键细节**：两份参考写高位写的都是 **0**，那是因为它们的 DMA 缓冲区在 4GB
以内（高位本就是 0）；而本项目缓冲区在 **4GB 以上**（第十二节已确认，代码里 `is64Bit=1`、开了
64 位寻址），高位**绝不能照抄那个 0**，否则续传地址会被打到错误的低 4GB 区域。正确做法是把
控制器已经自增好的低/高位都**读回来按真实值写回**，并把高位放在最后写（作为续传触发）。已按
这个口径改在 `bm1684xSdhci.c` 的轮询模式 DMA_END 分支里（共用引擎层，eMMC/SD 都受益；中断
模式的同名分支本来就有写高位，但同样写的是 0，若将来切到中断模式且缓冲区仍在 4GB 以上需一并
修正——已记录，暂未动，因为当前走轮询）。

**诚实标注不确定点**（避免重复前几轮"认定根因又被板子打脸"）：
- "写高位才是 64 位地址续传触发"这条是基于 SDHCI 规范行为 + 两份参考结构的推断，**尚未上板
  证实**；但它同时满足三个独立约束（解释 selftest 过/格式化卡、与两份参考结构一致、与 4GB
  以上缓冲区的事实自洽），是目前证据链最完整的一个，置信度高于前几轮。
- 退一步的【更稳兜底方案】仍然成立：若改高位后仍不行，就在 `bm_sd_core.c`/`bm_emmc_core.c`
  里把每次传输按 512KB 物理边界切块，保证没有任何单次 SDHCI 传输跨越边界 → DMA_END 永不触发
  → 这条重载逻辑根本不执行，绕开全部不确定性。这是 bulletproof 的退路，但代码量更大、每请求
  拆成多条命令，故先试小改（写高位）。
